...
- **Session 24 (2026-08-18):** App Review rejection fixes for submission b876a2fb (1.0 build 27) — see `docs/APP_REVIEW_RESPONSE_2026-08-17.md`. **5.1.1(iv):** capture-screen pre-permission button "Allow camera" → "Continue", descriptive message, and a denied-state branch that offers "Open Settings" instead of re-asking. **1.2:** `RingGauge` no longer falls back to `${percentile}%` (the blurred grid was showing "72%"/"61%" — the screen in the reviewer's screenshot); `BlurredTraitGrid` passes `scoreLabel`; "Ratings" tab label → "Progress"; "Re-rate now" → "Start a new scan"; "percentiles updated" → "scores updated"; capture screen states self-use only; clinical analyzing steps ("Measuring proportions"/"Checking symmetry") softened; "Attractive smile" → "Confident smile". Also corrected the Welcome trust line, which claimed "Photos stay on your device" while scans upload transiently — now "Photos are deleted after scoring". Stale e2e assertions on the removed "Top X% of men" copy fixed, and the screenshot/preview scripts follow the renamed tab. **2.3.6** (Age Rating → Age Assurance = None) and **2.1** (face-data reply) are App Store Connect actions documented in the response file.
