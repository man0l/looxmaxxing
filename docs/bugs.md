...
| F3 | Faster-feeling avatar preview: blurred/pixelized placeholder → sharp render (FE half) | 2026-07-15 | — | `AvatarRender` hard-cut from the SVG placeholder straight to the loaded `Image` on both Avatars screens. There's no low-res pass from the render backend yet (BE half — see BE3, still open), so this approximates a blur-up by de-blurring the loaded image itself: an `Animated.Image` fades in (opacity 0→1, native driver) while `blurRadius` steps down 14→8→3→0 over ~360ms, only triggered when a genuinely new `imageUrl` arrives (tracked via ref) so already-cached thumbnails still render instantly with no replay. |
