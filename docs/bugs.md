...
| — | Android rollout rejected: "does not allow any existing users to upgrade" | 2026-07-15 | — | `android-release.yml` computed `versionCode` from `github.run_number`, which is scoped per workflow file — when `deploy.yml` was deleted and recreated as `android-release.yml` (2026-07-12), its run counter restarted from 1 while Play Console already had higher `versionCode`s on record from the old workflow, so every build since regressed below what Play had already served. New `scripts/next-android-version-code.mjs` asks Play Console (via the Google Play Developer API, same service account used for upload) for the actual max `versionCode` across all tracks and uses `max(that + 1, run_number)`, so this can't regress again even if the workflow file is recreated once more. Falls back to `run_number` when Play can't be queried (first-ever release, no service account configured). |
...
| BE3 | Avatar render latency — real mid-generation preview | 2026-07-15 | — | The render Edge Function's `images.edit` call was a single blocking request with nothing to show until the full ~40-55s render finished. Switched to OpenAI's native `stream: true` + `partial_images` on the *same* call (GPT image models emit `image_edit.partial_image` SSE events with progressively-refined previews before the final `image_edit.completed`, ~100 extra output tokens per partial — no doubled cost/latency like a separate low-res call would be). **Follow-up (same day):** the first cut only ever showed one blurry frame before jumping straight to the final image, not a real Midjourney/Stable-Diffusion-style "picture appears, then sharpens" reveal. Fixed by uploading each partial to its own indexed path (`renders/<user>/<job>.preview-<0..2>.<fmt>`, matching OpenAI's own `partial_image_index`) instead of one overwritten path, bumping `OPENAI_IMAGE_PARTIALS` default 1→3 (the max), and having `GET /v1/renders/:jobId` return `previewIndex` alongside `previewUrl` so `submitRender()`'s poll loop can fire `onPreview` once per *new, strictly higher* index instead of once ever — comparing the index instead of the signed URL string, since that string differs every poll regardless of whether a new frame actually landed. **Scoped to `RENDER_MODE=async` only** — inline mode is a single blocking HTTP response with no polling loop to hand any preview through, so if Vercel's prod env still has `RENDER_MODE=inline` (the `.env.example` default), none of this runs at all; that's a deploy-config prerequisite, not a code bug. Backend PR: man0l/looxmaxxing-api. |
...
| B6 | Avatars tab gives no heads-up after "Delete my photos" — should say a new scan is needed | FE | After Profile → Privacy → Delete my photos, opening the Avatars tab shows nothing wrong: `AvatarsScreen` renders the full "Your goals"/"More to explore" gallery unconditionally — it never checks whether `latest.photoUri` exists, so every trait card looks normal (placeholder render, no photo). The only place that actually surfaces a message is deep inside `AvatarPreviewScreen`'s per-trait render effect ("Scan first to preview your potential.") — and only after the user taps into a *specific* trait card and its 300ms style-debounce render attempt runs, not at the tab's entry point. Should short-circuit at the `AvatarsScreen` level with an upfront "You don't have a photo yet — start a new scan" state instead of silently showing a normal-looking gallery and only failing per-trait after a delay. |
