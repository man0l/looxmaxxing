...
| F3 | Faster-feeling avatar preview: blurred/pixelized placeholder → sharp render (FE half) | 2026-07-15 | — | `AvatarRender` hard-cut from the SVG placeholder straight to the loaded `Image` on both Avatars screens. Fixed with an `Animated.Image` fade (opacity 0→1, native driver) + `blurRadius` step-down 14→8→3→0 over ~360ms, only triggered when a genuinely new `imageUrl` arrives (tracked via ref) so already-cached thumbnails still render instantly with no replay. **Follow-up (live-tested once BE3's streaming was fully deployed):** every new streamed frame (partial 1, partial 2, final) was replaying the *entire* reveal — opacity reset to 0 and blur reset to max each time — so instead of one continuous sharpening, the preview flashed back to blank/blurry before each successive frame, reported as "it renders, goes back to the main scan image, then renders again, closer." Fixed by only doing the full pop-in reveal for the first frame ever shown (tracked via a ref); every later frame just stays fully visible and snaps to unblurred, so successive frames read as one continuous "getting clearer" instead of repeated flash-resets. |
