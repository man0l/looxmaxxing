...
export async function runOnboardingToPaywall(page: Page) {
  ...
  await expect(page.getByText('Analyzing your photo')).toBeVisible({ timeout: 15_000 });

  await expect(page.getByText('Share your progress, your way')).toBeVisible({ timeout: 30_000 });
  await page.getByText('Continue', { exact: true }).click();
...
/** Unlocked Results shows the personal "6.1 / 10 · Solid" score line — match any. */
